...
main() {
    local theme
    theme=$(get_tmux_option "@minimal-tmux-theme" "terafox")
    ...
    bg=$(get_tmux_option "@minimal-tmux-bg" ${fox_fg})
    inactive_bg=$(get_tmux_option "@minimal-tmux-inactive-bg" ${fox_bg})
    ...
    status_left=$(get_tmux_option "@minimal-tmux-status-left" " #[bg=default,fg=${fox_fg},none]#{?client_prefix,,${indicator}}#[bg=default,fg=${fox_accent},bold]#{?client_prefix,${indicator},}#[bg=default,fg=default,bold] ")
    ...
    tmux set-option -g window-status-current-format "#[fg=${bg}]#[bg=${bg},fg=${fox_bg}]${window_status_format}#{?window_zoomed_flag,${expanded_icon},}#[fg=${bg},bg=default]"

    if [ "$show_expanded_icon_for_all_tabs" = true ]; then
        tmux set-option -g window-status-format "#[fg=${inactive_bg}]#[bg=${inactive_bg},fg=${fox_fg}]${window_status_format}#{?window_zoomed_flag,${expanded_icon},}#[fg=${inactive_bg},bg=default]"
    fi
}
